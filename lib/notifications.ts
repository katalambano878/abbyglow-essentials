...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@abbyglow.shop';
const EMAIL_FROM = process.env.EMAIL_FROM || 'AbbyGlow Essentials <noreply@abbyglow.shop>';
const BRAND = {
    name: 'AbbyGlow Essentials',
    color: '#0A0A0A',
    colorLight: '#FFFFFF',
    colorDark: '#F15A24',
    url: (process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3005').replace(/\/+$/, ''),
};
...
<td style="padding:6px 16px;color:${bold ? '#111827' : '#374151'};font-size:${bold ? '16px' : '13px'};font-weight:${bold ? '700' : '600'};text-align:right;">${value}</td>
    ...
    const plan = isDepositPlan(order.payment_plan) ? depositPlanLabel() : 'Full payment';
    const paid = Number(order.amount_paid || 0);
    const balance = Number(order.balance_due ?? Math.max(0, Number(order.total || 0) - paid));
...
${line('Payment plan', plan)}
${paid > 0 ? line('Amount paid', fmt(paid)) : ''}
${balance > 0.01 ? line('Balance on delivery', fmt(balance)) : ''}
...
export async function sendSMS({ to, message }: { to: string; message: string }) {
    ...
    try {
        ...
        const response = await fetchWithTimeout('https://api.moolre.com/open/sms/send', {
            ...
            body: JSON.stringify({
                type: 1,
                senderid: process.env.MOOLRE_SMS_SENDER_ID || 'ABBYGLOW',
                messages: [
                    {
                        recipient: recipient,
                        message: message
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9fafb;border-radius:12px;overflow:hidden;margin:16px 0;">
  ...
  ${emailInfoRow('Plan', isDepositPlan(order.payment_plan) ? depositPlanLabel() : 'Full payment')}
  ${Number(order.amount_paid || 0) > 0 ? emailInfoRow('Paid', `GH&#8373;${Number(order.amount_paid).toFixed(2)}`) : ''}
  ${Number(order.balance_due || 0) > 0.01 ? emailInfoRow('Balance due', `GH&#8373;${Number(order.balance_due).toFixed(2)}`) : ''}
  ${trackingNumber ? emailInfoRow('Tracking', trackingNumber) : ''}
</table>
