...
import { displayCompareAtPrice, displayDiscountPercent } from '@/lib/storefront-pricing';
import { isAppStorageUrl } from '@/lib/image-utils';
...
export default function ProductDetailClient({ slug }: { slug: string }) {
    ...
    async function fetchProduct() {
      try {
        ...
        const transformedProduct = {
          ...
          preorderShipping: getPreorderShippingNote(productData.metadata),
          isPreorder: isPreorderProduct(productData.metadata),
        };
  ...
  const compareAtDisplay = displayCompareAtPrice(product.compare_at_price, activePrice);
  const discount = displayDiscountPercent(product.compare_at_price, activePrice);
              ...
              <div>
                <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-100 mb-4 shadow-lg border border-gray-100">
                  <Image
                    ...
                    alt={product.name}
                    fill
                    unoptimized={isAppStorageUrl(product.images[selectedImage])}
                    ...
                    {product.images.map((image: string, index: number) => (
                      ...
                      >
                        <Image
                          ...
                          fill
                          unoptimized={isAppStorageUrl(image)}
                          className="object-cover object-center"
              ...
              <div className="flex flex-col xl:pl-8">
                <div className="flex flex-col mb-4">
                  ...
                  {product.isPreorder && product.preorderShipping && (
                    <div className="mt-4 inline-flex items-start gap-2 rounded-xl border border-brand-accent/30 bg-brand-soft px-4 py-3 text-sm text-brand-ink">
                      <i className="ri-time-line text-brand-accent mt-0.5 shrink-0"></i>
                ...
                <div className="flex items-baseline space-x-3 mb-8">
                  ...
                  {compareAtDisplay != null && (
                    <span className="text-lg text-gray-400 line-through decoration-gray-300">GH₵{compareAtDisplay.toFixed(2)}</span>
                  )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3.5 mb-10">
                  <button
                    ...
                    className={`flex-1 group relative overflow-hidden bg-brand-accent hover:bg-brand-accentDark text-white py-4 px-6 rounded-xl font-medium transition-all duration-500 flex items-center justify-center space-x-2.5 text-[15px] shadow-[0_4px_14px_rgba(0,0,0,0.1)] hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] hover:-translate-y-0.5 whitespace-nowrap cursor-pointer ${(activeStock === 0 || needsVariantSelection || needsColorSelection) ? 'opacity-50 cursor-not-allowed hover:-translate-y-0' : ''}`}
