    ...
    try {
      ...
      if (data) {
        ...
        const transformedProducts = data.map((p: any) => {
          ...
          const variantsCount = Array.isArray(variantEmbed)
            ? (typeof variantEmbed[0]?.count === 'number'
                ? variantEmbed[0].count
                : variantEmbed.length)
            : 0;
          return {
            ...p,
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            <span
                              className={`text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full ${
                                isPreorderProduct(product.metadata)
                                  ? 'bg-orange-100 text-brand-accent'
                                  : 'bg-emerald-50 text-emerald-700'
                              }`}
                            >
                              {isPreorderProduct(product.metadata) ? 'Pre-order' : 'Available'}
                            </span>
                          </div>
                        </div>
                      </div>
